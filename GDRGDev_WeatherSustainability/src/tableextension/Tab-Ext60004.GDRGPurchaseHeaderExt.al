tableextension 60004 "GDRG Purchase Header Ext" extends "Purchase Header"
{
    ...
    begin
        if (Rec."Buy-from Post Code" = '') or (Rec."Buy-from Country/Region Code" = '') then begin
            ClearWeatherData();
            exit;
        end;

        if WeatherHelper.FindZipCodeInfo(Rec."Buy-from Post Code", Rec."Buy-from Country/Region Code", Latitude, Longitude) then
            HasLocationData := true
        else
            if ZipCodeManager.GetZipCodeInfo(Rec."Buy-from Post Code", Rec."Buy-from Country/Region Code") then
                if WeatherHelper.FindZipCodeInfo(Rec."Buy-from Post Code", Rec."Buy-from Country/Region Code", Latitude, Longitude) then
                    HasLocationData := true;
