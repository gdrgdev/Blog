tableextension 60000 "GDRG Ship-to Address Ext" extends "Ship-to Address"
{
    fields
    {
    ...
    begin
        if (Rec."Post Code" = '') or (Rec."Country/Region Code" = '') then begin
            ClearWeatherData();
            exit;
        end;

        if WeatherHelper.FindZipCodeInfo(Rec."Post Code", Rec."Country/Region Code", Latitude, Longitude) then
            HasLocationData := true
        else
            if ZipCodeManager.GetZipCodeInfo(Rec."Post Code", Rec."Country/Region Code") then
                if WeatherHelper.FindZipCodeInfo(Rec."Post Code", Rec."Country/Region Code", Latitude, Longitude) then
                    HasLocationData := true;
